...
function generatePassword() {
  ...
  var optionUppercase =
    window.document.getElementById("capitalLetters").checked;
  var optionSmallLetters =
    window.document.getElementById("smallLetters").checked;
  ...
  if (
    optionCharacters == false &&
    optionNumber == false &&
    optionSmallLetters == false &&
  ...
  for (let i = 0; i < sizeKey; i++) {
    const randomMyPassword =
      myPassword[Math.floor(Math.random() * myPassword.length)];
